...
- `content-policy.json`: 結論の対象・掲載件数・2162専用注意書きとランキングタブ余白を固定する表示ポリシー
...
結論はMNPランキング1位の1キャンペーンだけから生成し、コード`2162`が1位のときだけ楽天アカウントへのログイン注意書きを表示します。ランキングタブ下の余白を含む正式なルールは`content-policy.json`が正本です。日次処理はこのファイルを変更できず、ランキング順位が変わった場合も同じ表示規則で結論を再生成します。詳細は`docs/campaign-content-policy.md`を参照してください。
